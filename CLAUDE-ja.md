...
- **エンジンが畳む先は媒体ではなく、一回の生成の容量である。** その容量はフォーマットカードが宣言する（時間軸を持たない一枚／十ページ／16:9 の三十秒）。静止画は**容量に時間軸がない場合**であって、エンジンの終着点では**ない**。出力が一枚の絵であることを前提にした規則を書いてはならない。
- **出力はイメージのための英語プロンプト**。その容量が要求する形をとる（静止画フォーマットなら SD/MJ 慣習、時間を持つフォーマットなら埋められた Wan 3.0 §1–20 の*仕様書*）。入力・指定は ja も可。話される台詞と画面に描かれる文字は解決言語のまま。
