...
> **方針メモ** —— 将来、**動画生成だけをこのリポジトリから切り離す**可能性が大いにある（画像生成は残す）。画像生成は生成自体に失敗することがほぼないが、動画生成は生成自体が失敗し、狙ったものが出ず、異物が混入する。一回一回の生成負荷も高く、トライアンドエラーで進むのに向いていないため。
...
**フォーマット 43 種 × 様式 54 種**——名前で再利用可。全索引（各カードの再利用 slug 付き）は [references/registry.md](references/registry.md)。
...
| 誘引 | サムネイル · ギャグ漫画 · 4コマ漫画 · 横長ストリップ · 1コマ漫画（風刺） |
...
| メディウム | 様式 |
|---|---|
| 版画・印刷・タイポグラフィ | 木版 · リノカット · 銅版画 · リソグラフ · 厳粛な台帳 · 漫画CMYKハーフトーン · スイス・インターナショナルタイポグラフィ |
| 手描き | 水彩 · 油絵 · 水墨画 · 手書き抽象 · 鉛筆 · 木炭 · パステル · スケッチ · マンガ線画 · クリーンラインラボ · 青図・設計図 · レシピ・手順カード · 回路図 · 機構・働きの図 · 定規とコンパスの作図 · 実験ノート · 少女漫画調 · 劇画調 |
| デジタル | ピクセル · PS1 低ポリゴン · バウハウス系ベクター · フラットコミック · ラインナート · アイソメトリック · ダークグロー・ミニマルベクター · ハイテンション日常系コミック · セル画調 · ウェブトゥーンソフトレンダー · 少年バトル調 · フラットデザイン · マテリアルデザイン · ネオブルータリズム |
...
想定目的（理解/伝達/誘引/再体験/記録/装飾/デザイン）がフォーマットを決め、フォーマットが圧縮の粒度×時間を決める。詳細は `references/types.md`。
...
- [references/registry.md](references/registry.md) — 名前付きカードの索引（フォーマット 43 種 · 様式 54 種）
