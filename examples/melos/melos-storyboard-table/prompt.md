...
## フォーマット（Format）
ストーリーボード `table`：カット番号／絵／内容（動作＋セリフ＋秒数）の縦3列表、上→下。絵の列は他モードと同じビート・ショット種別・カメラ語を持つ小さな 16:9 のコマ。**秒数は入力自身の重点を反映して不均等**——原作が最も長く筆を費やす「倒れてから泉で立ち上がるまで」を最大の 8 秒に置き、出発は 2 秒で流す（計 30 秒＝時間への手渡し）。
