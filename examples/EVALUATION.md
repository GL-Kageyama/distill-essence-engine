...
| ケース | 入力 | フォーマット | 様式 | 目的 | 判定 |
|---|---|---|---|---|---|
| gozen-niji-character-sheet | 『午前二時に、あなたは誰の時間を生きていますか』（シリーズ定数） | **キャラクター設定画（character-sheet・新カード）** | セルシェード | 制作仕様 | **PASS**（anti-gen 2/10・aesthetic 8/10・admiration 8/10） |
...
- **検証パターン＝フォーマット差し替え 4件＋様式差し替え 1件**。同一入力（『午前二時に、あなたは誰の時間を生きていますか』シリーズ定数）で設定画→キャラボード→コンセプトボード→美術ボードの順にフォーマットだけを差し替え、制作仕様の4分類が競合せず機能することを確認。melos-luminous-anime は同一フォーマット（concept-board）× 様式だけ luminous-anime に差し替え。**全5枚 PASS → レジストリへ登録**（フォーマット 31→35、様式 49→50、検証待ち解除）。
...
| ケース | 入力 | フォーマット | 様式 | 目的 | 判定 |
|---|---|---|---|---|---|
| gozen-niji-location-board | 『午前二時に、あなたは誰の時間を生きていますか』（シリーズ定数） | **ロケーションボード（location-board・新カード）** | ソフトセルアニメ | 制作仕様 | **PASS**（anti-gen 2/10・aesthetic 8/10・admiration 弱PASS 49） |
...
- **検証パターン＝フォーマット差し替え 3件**。同一入力（『午前二時に、あなたは誰の時間を生きていますか』シリーズ定数）・同一様式（soft-cel-anime）で、0.1.28 の4種（設定画→キャラボード→コンセプトボード→美術ボード）に続く第2弾の3種——ロケーションボード（全場所の地理）→キーポーズボード（一主体の動作語彙）→シーンボード（一場面の主ステージング）——が、0.1.28 の既存4種と競合せずに別々の板として機能することを確認。**全3枚 PASS → レジストリへ登録**（フォーマット 35→38、検証待ち解除）。
...
## 午前二時に、あなたは誰の時間を生きていますか 第1話 → storyboard table（PASS 3/3・0.1.32 追補）
...
## 午前二時に、あなたは誰の時間を生きていますか 第2〜12話 → storyboard table（全12話 PASS 3/3・0.1.32 追補）
...
## semantic-visual-loom → ヒーロー画像（PASS 3/3・0.1.38 追補）

実在のリポジトリケース：`semantic-visual-loom-hero`。**入力＝semantic-visual-loom の README・docs/usage.md・CLAUDE.md の本質（input.md）・様式＝ダークグロー・ミニマルベクター**。姉妹リポジトリを入力にした hero の第2例（1例目 `semantic-audio-loom-hero`）。⚠️ **中央シンボルは織機ではない**——姉妹が既に織機を立てているため、この基盤自身の主張（**「10 ショットは、台帳 1 枚から出ている」**）から**制作台帳 1 本**に畳んだ。同じ家族に同じ記号を 2 度置くと、どちらの固有性も消える。

- 判定者：anti-generic-filter・aesthetic-critic・admiration の3系統（1ケース×3＝3判定・並列）。判定対象テキスト（Merged プロンプト）をエージェントプロンプトに直接埋め込み、ファイル/ツール読取禁止、model='sonnet'（0.1.20〜0.1.32 と同手順）。**両回とも3系統が通常どおり判定を返した**（general-purpose への代替なし・`tool_uses: 0` は読取禁止の指定どおりで崩れではない）。

**第1回（初稿）**

| ケース | 配置×様式 | 判定 |
|---|---|---|
| semantic-visual-loom-hero | `hero`×dark-glow-vector | **PASS**（anti-gen 1・aesthetic 8・admiration 42 弱PASS） |

- **精錬（③方向・1回）**：aesthetic-critic が**中核基準そのものへの指摘**を返した——理由節と総括が「見る側に委ねる」前に意味を渡している（`because every frame is an independent generation…` の理由節・`This unbroken thread is the whole point: …` の総括・`what must not be shown, stopped while it is still only a specification` の註釈）。SKILL.md の Output 規則（合成プロンプトに註釈・説明を加えない）とも一致するため、**見る側へ向けた説明だけを剥がした**——視覚的な事実（継ぎ目・不揃いのコマ幅・コマごとの印 1 つ・切れない糸・脇へ逸れる 2 本目・白熱の一点・余白）は**1 つも削っていない**。先例に倣う（bocchan-gag-manga は PASS 78 後に背景スロットを充填して再判定、youtube-loop-engineering-manga は PASS 84 後に④一貫性を反映）。

**第2回（精錬後・これが現在の `prompt.md` の Merged）**

| ケース | 配置×様式 | 判定 |
|---|---|---|
| semantic-visual-loom-hero | `hero`×dark-glow-vector（精錬後） | **PASS**（anti-gen 2・aesthetic 8.5・admiration 40 弱PASS） |

- **⑧忠実性**：**両回・全6判定で「⑧違反なし」**。台帳＝1 冊／1 本のファイル（分割しない）・継ぎ目の見える分離したコマ・コマごとに印はただ 1 つ（1 ショット＝1 変化）・コマ幅の不揃い（長さは従属変数）・台帳の背から出て切れない糸（連続性と開示を一緒に持つ 1 本）・帯に届く前に脇へ逸れて暗に終わる 2 本目（禁制集合＝描かれる前に止まる）・タグラインは README の逐語。精錬は説明の削除のみで、事実の追加も改変もしていない。第1回 admiration が唯一境界として挙げたのは「右へ行くほど帯がまばらになる」点だが、「収束先の作品への強調であって、リポジトリについての主張ではない」として捏造に数えないと判定された。
- **admiration の所見**：42 → 40（両回とも弱PASS＋⑧違反ゼロ）。超出を担うのは**脇へ逸れる 2 本目の糸**——評価者は「明らかな図なら台帳 1 冊が帯 1 本を送るはずで、そこから**2 本**が出て一方が暗に終わる、という超出」を名指しし、精錬後は「分割しないという方針が、註釈なしで見えるようになった」と述べた。白熱の一点への収束は左→右の掃き出しで予告されており、驚きではなく端正な終止として着地する（仕様型出力の構造的低さ）。
- **aesthetic-critic の所見**：8 → **8.5**（≥7）。精錬で**本文への指摘が消えた**——「プロンプトのどこにも説明されておらず、読む側に委ねられている」。2 本の糸の対比（連続性は通し、捨てる側は暗へ逸らす）が「状態は生成の外にある」を註釈なしで可視化している点、コマ幅の不揃いが「長さは従属変数」を語なしで示す点、4 色規律が最小性を担う点を挙げた。
- **anti-generic の所見**：1 → 2（閾値≤3・シグネチャ無）。一般化できない具体は変わらず——脇へ逸れる 2 本目の糸（禁制集合に負の空間を与えた点が最も固有）・不揃いのコマ幅・コマごとにただ 1 つの印・`#f5b14e`/`#ff8c42` と `#081828`/`#080828` の固定値。両回とも評価者は**残余の汎用を内容でなく枠**（hero の器そのもの・`elegant tech aesthetic` の一句）に限局した。様式・否定句の繰り返しは⑦機能ガード（判定除外）。
- ⚠️ **残る所見（直さず報告するもの・4 件）**：
  - **タグライン**：aesthetic-critic は「枠の中に答えを書いてしまう」として、タグラインを弱めるか落とすことを最善の是正に挙げた。しかし**タグラインは様式カードの必須スロット**である——`references/styles/dark-glow-vector.md:11` が `TITLE`/`TAGLINE` を環境変数として持ち、同 `:18`・`:22`・`:45` が「左上に薄灰サンセリフのタイトル＋タグライン」を視覚分解として定める（`references/formats/hero.md:14` は「No text, or text spelled correctly once」）。**1 ケース側で落とせる要素ではない**ため変更していない。⚠️ これは**カード水準の緊張**である——様式カードがタグラインを義務づけ、評価者が「タグラインは間接性に反する」と言う。カードを動かすかは著者の判断。**→ 著者判断（2026-09-14）：残す。** タグラインは様式カードの必須スロットであり、hero 3 例が同一の型を使っている。指摘はカード水準の緊張として記録に留める。
  - **二重強調（評価者間で読みが割れた）**：`exactly one small amber-gold mark, and only one` の念押しを、aesthetic-critic は「形が単一性を担保することを信じていない、唯一自信ではなく念押しになった箇所」と読み、anti-generic-filter は逆に**固有性の側**に数えた（コマごとにただ 1 つ、という計数可能な規則）。⚠️ エンジン自身の ⑦ は「数を明示する」側に立つ（hero カード `formats/hero.md:19` も「No text, or spell it correctly once」）ため**変更していない**。**同じ 3 語を 2 系統が逆に読んだ**という事実を記録する。
  - **姉妹ケースの同型**：`semantic-audio-loom-hero` の Merged も同じ型を持つ（`This single hidden thread is the whole point — what is unspoken becomes intonation…`）。**今回剥がしたのと同種の文**であり、初稿はこの家族の型を写していた。⚠️ 同ケースは生成済み（フォルダに PNG がある）で本件の作業対象ではないため触っていない。
  - **姉妹ケースの判定記録が無い**（実測）：`examples/EVALUATION.md` に `semantic-audio-loom-hero` の文字列は **0 件**、case folder にも EVALUATION が無い。**判定を記録しないまま `examples/` に入った hero が 1 件ある**という事実の報告に留める。

## 『ハビッツ！！！』制作状態 → 新フォーマット 進捗盤（第三枚 PASS 2系統・0.1.47）

実在の作品ケース：`examples/habits-status/`。**入力＝制作の状態そのもの**（`input.md` は出典の指し先と実測だけを持ち、内容は複製しない）。**新しいフォーマット `progress-board` を、この樹のために起こして検証し、登録した。** 主題は制作の中身ではなく**制作の状態**であり、**数値を一枚も入れずに**「どこまで進んだか」を表す。物差しは外から借りず、作品の【ルック】が既に定めている**三つの解像度**（印刷・ボールペン・鉛筆）をそのまま使う。

**三枚を書いた。** 一枚＝全量（俯瞰・一冊に畳む）／二枚＝現在地（接写）／三枚＝目録（世界観の要素とモチーフを、出典が「運搬」として読んでいる順に一列へ開く）。**三枚目の制約が最も強い——枠内に文字を一つも置かない**（作者の指示）。一・二枚は進みを「書かれた字の解像度」で示せたが、三枚目はそれができない（列に並ぶ物の上に読める印が無い）ため、**解像度は字ではなく「その物がどう作られたか」へ移した**——**まだ戻せる／もう戻せない／機械が刷った**の可逆性である。

- 判定者：anti-generic-filter・aesthetic-critic の 2 系統（1ケース×2）。判定対象テキスト（Merged プロンプト）をエージェントプロンプトに直接埋め込み、ファイル/ツール読取禁止、model='sonnet'（0.1.20〜0.1.38 と同手順）。**⚠️ 美的判定の第1回は、この樹で既知の評価者の崩れ方（`tool_uses: 0`・`｜｜DSML｜｜` の疑似ツール呼び出し）で返ったため、`general-purpose` エージェントが評価者の定義ファイルを読んで代替判定した**（0.1.28 の 15 判定と同手順。**代替であることをここに明記する**）。以降の周回は通常どおり返った。

**スコアの推移（第三枚「目録」）**

| 周回 | anti-generic | aesthetic |
|---|---|---|
| 1 | 70 | 74（**⚠️ 旧版を読んだ判定**。精錬前のファイルを測っている） |
| 2 | **76 PASS**（genericness 2・シグネチャ無） | 71 |
| 3 | 74 PASS（genericness 3・シグネチャ無） | 74 |
| 4 | 77 **FAIL**（シグネチャ 2 本） | 78 |
| 5 | 82 **FAIL**（シグネチャ ` one thin bright ` 1 本・17字） | **64**（四つの次元が一様に13〜16点落ちた） |
| 6（最終・凍結） | **84 PASS**（genericness 2/10・**計上シグネチャ 0件**） | **74 PASS** |

- **閾値は二条件である**——**72以上かつ紋切型シグネチャなし**（[`EVALUATION.md:10`](EVALUATION.md) ほか）。美的は **7/10（＝70）**。**第三枚は両方を満たした。**
- **⚠️ 第5周の美的 64 は、私が第4周で作った綻びが原因である。** 列を「遠い縁の手前で止める」形にした結果、同じフレームの上端で三つが同時に主張される状態が生まれ（列の終端・未達パッチを切る枠の縁・机の縁）、**二人の判定者が独立に同じ箇所を指した**。列に横向きの方向を与えて解いたところ `formal_coherence` 65→76・`expressive_range` 62→76 が戻った。**⚠️ `sensuous_quality`（73）と `aesthetic_ambition`（74）は、78／84 の山へは戻らなかった**——原因（俯瞰・光源一本・`held equally sharp from end to end`・埃を落としたこと・`even tone`）は、この修正が触っていない側に在る。
- **⚠️ 計上シグネチャは周回で反転した（この樹で二度目）。** 同一系統の判定者が、`its fall is broad and flat` という**同一文字列**を、ある周回では「共有の光源リグの記述であり移植可能な内容句ではない」として除外し、次の周回では「**31字の一致**」として計上した。**点数は 70→76→74→77→82→84 と動き続け、シグネチャ条件だけが FAIL を出し続けた。** ゆえに**残件一件を「あと一周で通る」と読まない**——**前周回の「これは除外した」を根拠に残してよいと判断しない。** **ただし自分が今作った流用は分散ではなく実在の欠陥である**——この樹では `taking the tube in one thin bright thread`（姉妹の `...in one thin bright line` から名詞だけ差し替えた）がそれにあたり、直した。
- **⑧忠実性**：出典の語のまま置いたもの＝三つの解像度とその意味・【ルックとカメラ】の「カメラが置けるのは手・物・顔」・運搬の順（run-10【動きの単位】）・物の疵（出席簿の左袖の厚み・綴じ束の四つの切り離し跡・宛名票の重なりと糊・預かり札の鉛筆・名札のプラスチック）。**第三枚が世界観に与えないもの**＝題の「！」三個（置けば字になる）・三つの掟・四つの通路・禁則の語群（**物を与えられていない**ので列に置かず、光と仕上がりの条件としてだけ現れる）。**③翻訳であると申告した装置**＝本文ゼロ・映像ゼロの二行を、摩耗した面の逆（**手の渡っていない、木目がまだ木にあるパッチ**）として机の側に置いたこと（出典の物ではない）。
- **機械検算（凍結時の実測）**：8固まり・**複数行の固まり 0**・肯定ブロック 916語・**負の一覧 68項**。文字を名指す語（`writ` `letter` `word` `name` `numeral` `digit` `kanji` `kana` `label` `text` `sign`）は**全て 0**。姉妹二枚との n-gram 共有は **n=8 で 0 run・n=7 で 0 run**、n=6 で残るのは様式の忠実性アンカー `magenta and gold against deep cyan,` 一句のみ（申告済み）。**⚠️ この検算器は、仕込んだ探究（n=6 で 16 run を検出）で鳴ることを確かめてある**——0 は「相手が空だから 0」ではない。
- ⚠️ **残る所見（直さず報告するもの・3 件）**：
  - **左端（未手当て）**：列が枠の外から入るため、**作られた物も枠の縁を越えている**——カード自身の規則（作られた物は枠の内側で終わり、越えるのはまだ作られていない側だけ）と逆である。**手当ては一行**（`beginning inside the frame near its left edge`）で、**美的判定 #5 が「この一枚がいま崩れるとすれば一点——左の縁、列が入ってくる側」として挙げた箇所**である。**英文を動かせば anti-generic の PASS が無効になるため、この周回では当てていない**（カードの「出所」節と、その盤の申告に記録した）。
  - **光の段の二段説明**：`so the row reads as one lit strip` と `that unbroken run is what makes this read as one row` の二段を、**美的 #4 は「統合の担い手」として強みに数え、anti-generic #5 は冗長として計上側に置いた。** 判定者間で読みが割れたため、**同じ 3 語を 2 系統が逆に読んだ事実の記録に留め、変更していない**（英文を動かせば PASS が無効になる）。
  - **三段の範囲**：`Every thing on this surface stands in one of three states` は、机の側の**手の渡っていないパッチもその射程に入れる**。パッチは「物」ではないため厳密には綻びだが、**その一文が無いと三段が物だけに閉じて未達の担い手が宙に浮く**ため、この形を採った。
- ⚠️ **姉妹二枚（第一枚「全量」・第二枚「現在地」）は、閾値を満たさずに終わっている。****美的は 70→75（≥7 で PASS）だが、anti-generic は 63→62→66 と推移して 66 で FAIL**（≥72 かつシグネチャなし）。**主因は実測で名指しされた二点**——接写が俯瞰の art direction をほぼ言い直していること（**39語の逐語一致**）と、`fine dust drifting` が両方に残っていること（検出済みシグネチャ）。**作者が「これはこれでいい」と述べてこの二枚をそのまま置いたため、登録の根拠は第三枚の PASS に置いた。** この二枚の記録はここまでとする（作り直していない）。
- **登録物**：`references/formats/progress-board.md` ×3言語（ja・zh はミラー。**プロンプトテンプレは三言語でバイト一致**＝英語不変部）＋`registry.md` ×3言語に 1 行ずつ。**あわせて件数を持つ全箇所を実数へ合わせた**——**フォーマット 43→44**（README ×3言語の2箇所ずつ・`docs/index.md` ×3言語・`examples/README.md` ×3言語）と、`types.md` ×3言語の制作仕様ファミリー（一覧・粒度×時間・サイズ）。README ×3言語の目的別フォーマット表にも `progress board` を足した。**⚠️ 実数は「registry の行数」であり、ディスク上のファイル数ではない**（下の注記を参照）。
- **⚠️ 登録はローカルである。`git push` はしていない**（この樹の規則）。commit も著者の指示を待つ。
...
- semantic-visual-loom-hero は初回 PASS（anti-gen 1・aesthetic 8・admiration 42）後、aesthetic-critic が指摘した**本文の説明過多**（理由節 `because …`・総括 `This unbroken thread is the whole point: …`）を剥がして再判定した（**PASS**／anti-gen 2・aesthetic **8.5**・admiration 40・⑧違反ゼロ）。⚠️ 同評価者が残したタグラインへの指摘は**様式カード `dark-glow-vector` の必須スロット**（`TITLE`/`TAGLINE`）に由来するため変更していない——**様式カードがタグラインを義務づけ、評価者がタグラインを間接性に反するものとする**という緊張は、この様式の hero すべてに掛かる（姉妹ケースも同型の説明過多を持つ。詳細は本文）。
...
- ⚠️ **未登録のカードがディスク上に 1 枚ある（実測・直さず報告）**：`references/formats/episode-board-v2.md`。**登録済みの `episode-board` とは別ファイルで、registry.md ×3言語のどこからも参照されていない**（`episode-board-v2` の文字列は registry ×3言語で 0 件）。**ゆえに「registry の行数」と「formats/ のファイル数」はずれる**——0.1.47 の時点で **登録 44 行・ファイル 45 枚**。**これは私が作ったものではない**（0.1.47 の登録前から在り、当時の実数は登録 43・ファイル 44）。**登録するか捨てるかは著者の判断**であり、この項では動かしていない。
- ⚠️ **`examples/README.md` ×3言語のカバレッジ行列に、登録数のずれが残っている（実測・直さず報告）**：行列の文は「登録 N 種のうち 33 種」と書きながら、**括弧内に 34 のフォーマット名を並べている**（`episode board` を含む）。0.1.47 の件数是正で分母は 44 になったが、**分子の 33 と名前の数 34 の食い違いは 0.1.47 以前からのもので、どちらが正かは私には決められない**（「例示されている」の数え方の定義による）。**`progress board` はこの行列に足していない**——`examples/habits/`・`examples/habits-status/` は **`examples/README.md` のケース一覧に元から載っていない**（キャラクター設定画 28 枚の樹も同様）ため、この樹だけを足すと索引の規則が変わる。
