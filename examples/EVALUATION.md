...
- **結果：35/35 PASS（スコア 76–90）・失敗 0**（修正・削除なし）
...
- URL 入力（YouTube 動画／ホームページ／GitHub リポジトリ）は、実 URL の検証ケースが 3 件ある。いずれも `scripts/fetch.py` で実フェッチした内容を入力に使用し、カバレッジ行列の YouTube 動画 × 理解（解説図）・× 記録（漫画）・GitHub リポジトリ × 誘引（ヒーロー画像）セルを実 URL で埋めた。
