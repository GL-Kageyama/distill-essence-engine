...
> **デザイン目的**——`poster`／`cover`／`hero`／`icon` はデザイン目的のフォーマットでもある（第一義はデザイン）。ここでは誘引の例示に使っている。**デザインは別の入力軸**——その入力はブランド・製品・アプリであり、コンテンツではない。以下がその検証ケース。

**デザイン・カバレッジ**（入力＝ブランド／製品——distill-essence-engine 自身。フォーマット差し替えは様式＝スイス型固定・様式差し替えはフォーマット＝アプリ画面固定）：

| 入力＼軸 | アプリ画面 | ランディングページ | ワイヤーフレーム | ブランドボード | 名刺 |
|---|---|---|---|---|---|
| フォーマット差し替え（ブランド×スイス型） | [design-format-app-screen](design-format-app-screen/) | [design-format-landing-page](design-format-landing-page/) | [design-format-wireframe](design-format-wireframe/) | [design-format-brand-board](design-format-brand-board/) | [design-format-business-card](design-format-business-card/) |
| 様式差し替え（ブランド×アプリ画面） | [swiss-style](design-style-swiss-style/) · [flat-ui](design-style-flat-ui/) · [material-ui](design-style-material-ui/) · [neubrutalism](design-style-neubrutalism/) | — | — | — | — |

**FF ブランドボードシリーズ**——同じ brand-board×スイス型の処理を10の実在ブランドに当てた、1ナンバリング＝1板の10枚組。各板は Final Fantasy I〜X を、その作品の**非キャラ必須モチーフ**だけで想起させる。**キャラクターは一切描かない**（主人公・敵・顔は当然として、シルエットは抽象的な形でマークに尺度を与える場合のみ）。ユーザー要望により各板はスイス・インターナショナルタイポグラフィで**展開例の欄なし**——板はマーク（モチーフをフラットな色面に）・パレット・タイポグラフィだけを持つ。名刺・パッケージ・製品モックアップは置かない。パレットはマークの中と単一アクセントとして存在し、色見本の行や描画された色名・hexコードの文字では決して描かない——画像内の文字は見出しと英語サブタイトルだけ。板の差は入力（ブランドブリーフ）だけ。フォーマットと様式は固定。

| 板 | 核モチーフ（キャラなし） | 板 | 核モチーフ（キャラなし） |
|---|---|---|---|
| [ff1](ff-brand-boards/ff1/) | 四つのクリスタル・光と闇 | [ff6](ff-brand-boards/ff6/) | 魔石・世界崩壊 |
| [ff2](ff-brand-boards/ff2/) | ワイルドローズ・帝国 | [ff7](ff-brand-boards/ff7/) | 魔晄・メテオ・ミッドガルの円盤 |
| [ff3](ff-brand-boards/ff3/) | 雲の上のクリスタルタワー | [ff8](ff-brand-boards/ff8/) | ガーデン・月 |
| [ff4](ff-brand-boards/ff4/) | 月・赤い翼 | [ff9](ff-brand-boards/ff9/) | クリスタル・霧 |
| [ff5](ff-brand-boards/ff5/) | 虚空・縫合された二世界 | [ff10](ff-brand-boards/ff10/) | 水・螺旋 |

**様式カバレッジ**：レジストリ登録様式は **54 種（全て検証済み）**。様式差し替えテスト（[verify-rich/](verify-rich/)）で新規 15 様式・7 フォーマットを例示・検証した。3 様式（claymation／cyberpunk／vaporwave）は検証で**不採用**となりレジストリ外（詳細は [verify-rich/EVALUATION.md](verify-rich/EVALUATION.md)）。ダークグロー・ミニマルベクター（実績 hero ファミリー・紺＋金橙＋白熱の4色）は 0.1.17 の distill-hero で追加。ハイテンション日常系コミック（ギャグ漫画・日常系アニメ）は 0.1.18 の bocchan-gag-manga（青空文庫『坊っちゃん』→10 ページ漫画）で追加。クリーンラインラボ（教科書の実験図の明快さ×静かなかわいさ）は 0.1.19 の ai-schrodinger-cat で追加。マンガ様式 6 種（セル画調／少女漫画調／ウェブトゥーンソフトレンダー／少年バトル調／劇画調／漫画CMYKハーフトーン）は 0.1.21 の melos-* スタイル差し替えケースで追加（初のカラーマンガ様式。漫画CMYKハーフトーンは初の漫画向け印刷工程様式）。実写系（写真・映画）様式 7 種（ドキュメンタリー写真／スタジオポートレート／大判風景写真／マクロ写真／シネマティックスチル／1970年代ニュー・ハリウッド／インスタント写真）は 0.1.22 の melos-* スタイル差し替えケースで追加（初のレンズと光のメディウム様式。ストリートフォト・フィルムノワールと並んで実写軸を完成）。機能文書ファミリーの様式 6 種（青図・設計図／レシピ・手順カード／回路図／機構・働きの図／定規とコンパスの作図／実験ノート）——clean-line-lab の兄弟で、それぞれが概念を自分自身の文書（計画／手順／接続／働き／証明／記録）で描く——は 0.1.24 の power-* スタイル差し替えケース（同一入力 × 概念図固定 × 様式だけ差し替え）で追加・検証済み。実演は下記の distill-* 出力サンプル。光の写実アニメ（高彩度の夕景・体積光——soft-cel-anime の意図的な逆）は 0.1.28 の melos-luminous-anime で追加。デザイン系譜の様式 4 種——スイス・インターナショナルタイポグラフィ（モジュラーグリッド・左寄せグロテスク・アクセント1色）／フラットデザイン（平らな単色面・グラデなし・影なし）／Material Design（elevation の面・8pt グリッド・FAB 1つ）／ネオブルータリズム（太い黒の輪郭・ハードオフセット影・ぶつかる彩度）——は 0.1.30 の design-* 様式差し替えケース（同一入力 × アプリ画面固定 × 様式だけ差し替え）で追加・検証済み（ブランドの固定パレットは様式の既定アクセントを置き換えられる——スイスの赤→ブランドの金橙）。

**フォーマットカバレッジ**：レジストリ登録フォーマットは **44 種（全て検証済み）**。verify-rich/ で新規 7 種（ストーリーボード／ポスター／絵本／ロゴ／エンブレム／スプライト／タロットカード）を検証・昇格した。ヒーロー画像（hero・16:9 バナー）は 0.1.17 の distill-hero で追加。ギャグ漫画（多コマ・4ビートリズム）は 0.1.18 の bocchan-gag-manga で追加。説明系フォーマット 4 種（タイムライン／フローチャート／比較マトリクス／データチャート）は 0.1.20 の attention-* ケースで追加。マンガフォーマット 8 種（4コマ漫画／ウェブトゥーン／横長ストリップ／見開きスプラッシュ／1コマ漫画（風刺）／エッセイ漫画／ルポ漫画／学習漫画）は 0.1.21 の melos-*／attention-educational-manga フォーマット差し替えケースで追加。概念図（conceptual illustration：ビジュアルメタファー／スプリットコンポジション／カラーコーディングで全概念を一枚の象徴の絵に折り込む）は 0.1.23 の concept-average-vs-particular で追加。アニメ・プリプロダクション系フォーマット 4 種（キャラクター設定画／キャラクター イメージボード／イメージボード（concept-board）／美術ボード）は 0.1.28 の gozen-niji-* ケースで追加（後工程が再現するための制作仕様であって、鑑賞する絵ではない）。さらにアニメ・プリプロダクション系フォーマット 3 種（ロケーションボード／キーポーズ・アクションボード／シーンボード）は 0.1.29 の gozen-niji-* ケースで追加（物語の地理／一人の主体の動きの語彙／一場面のマスター・ステージング）。**デザイン**目的を 0.1.30 で新設（第7の想定目的。入力はブランド・製品・アプリであり、コンテンツではない）。デザインフォーマット 5 種（アプリ画面／ランディングページ／ワイヤーフレーム／ブランドボード／名刺）は design-* フォーマット差し替えケース（同一入力 × スイス型固定 × フォーマットだけ差し替え。ブランドの蒸留メタファーが各レイアウトの文法として生き残る）で検証。既存のマーク系 7 枚（ロゴ／アイコン／エンブレム／ポスター／表紙／ヒーロー画像／タロットカード）は `象徴（デザイン）` に再分類。カバレッジ行列は登録 44 種のうち 33 種（解説図／インフォグラフィック／表紙／アイコン／サムネイル／ポスター／絵本／漫画／挿絵／イメージボード／ストーリーボード／スプライトシート／タロットカード／ヒーロー画像／ギャグ漫画／エピソードボード／タイムライン／フローチャート／比較マトリクス／データチャート／概念図／キャラクター設定画／キャラクター イメージボード／イメージボード（concept-board）／美術ボード／ロケーションボード／キーポーズ・アクションボード／シーンボード／アプリ画面／ランディングページ／ワイヤーフレーム／ブランドボード／名刺）を例示する（マンガフォーマットは下記の melos-*／attention-educational-manga ケースで例示）。
...
| design-format-brand-board | distill-essence-engine ブランド（ブランドブリーフ） | ブランドボード | スイス・インターナショナルタイポグラフィ | デザイン |
| design-format-business-card | distill-essence-engine ブランド（ブランドブリーフ） | 名刺 | スイス・インターナショナルタイポグラフィ | デザイン |
| design-style-swiss-style | distill-essence-engine ブランド（ブランドブリーフ） | アプリ画面 | スイス・インターナショナルタイポグラフィ | デザイン |
| design-style-flat-ui | distill-essence-engine ブランド（ブランドブリーフ） | アプリ画面 | フラットデザイン | デザイン |
| design-style-material-ui | distill-essence-engine ブランド（ブランドブリーフ） | アプリ画面 | Material Design | デザイン |
| design-style-neubrutalism | distill-essence-engine ブランド（ブランドブリーフ） | アプリ画面 | ネオブルータリズム | デザイン |
| ff-brand-boards/ff1 | ファイナルファンタジーI（四つのクリスタル・キャラなし） | ブランドボード | スイス・インターナショナルタイポグラフィ | デザイン |
| ff-brand-boards/ff2 | ファイナルファンタジーII（ワイルドローズ・キャラなし） | ブランドボード | スイス・インターナショナルタイポグラフィ | デザイン |
| ff-brand-boards/ff3 | ファイナルファンタジーIII（クリスタルタワー・キャラなし） | ブランドボード | スイス・インターナショナルタイポグラフィ | デザイン |
| ff-brand-boards/ff4 | ファイナルファンタジーIV（月・キャラなし） | ブランドボード | スイス・インターナショナルタイポグラフィ | デザイン |
| ff-brand-boards/ff5 | ファイナルファンタジーV（虚空・キャラなし） | ブランドボード | スイス・インターナショナルタイポグラフィ | デザイン |
| ff-brand-boards/ff6 | ファイナルファンタジーVI（魔石と世界崩壊・キャラなし） | ブランドボード | スイス・インターナショナルタイポグラフィ | デザイン |
| ff-brand-boards/ff7 | ファイナルファンタジーVII（魔晄とメテオ・キャラなし） | ブランドボード | スイス・インターナショナルタイポグラフィ | デザイン |
| ff-brand-boards/ff8 | ファイナルファンタジーVIII（ガーデンと月・キャラなし） | ブランドボード | スイス・インターナショナルタイポグラフィ | デザイン |
| ff-brand-boards/ff9 | ファイナルファンタジーIX（クリスタルと霧・キャラなし） | ブランドボード | スイス・インターナショナルタイポグラフィ | デザイン |
| ff-brand-boards/ff10 | ファイナルファンタジーX（水と螺旋・キャラなし） | ブランドボード | スイス・インターナショナルタイポグラフィ | デザイン |
