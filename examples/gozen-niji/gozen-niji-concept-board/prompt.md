...
> **対照ケース**：[gozen-niji-art-board](../gozen-niji-art-board/prompt.md) は**同じ入力・同じ様式**で美術ボードを作っている。差はフォーマットだけ——こちらは「世界の空気」、あちらは「一場所の基準」。

## 内容（Content）

②選択＝**12話ぜんぶの温度**であって、どの話の出来事でもない。この作品の空気は「画面の光だけが灯っている部屋」——それは第1話の設定であると同時に、全12話が最後まで手放さない照明条件である。第12話で真白が自分の指で打つときも、光源は変わらない。だから一枚に畳むべきはこの照明そのもの。

③翻訳＝**人物を主役にしない**。布団の起伏と、画面の外へ落ちる手の甲だけが人の存在を示す。顔は描かない。虹色はガラスの内側から**わずかに外の暗い空気へ滲む**（光線でもパーティクルでもオーラでもない——第4話以降の規則）。

**⑧ガード**：ニジを出さない。第1話時点でニジは不在であり、この一枚は特定の話ではないが、**世界の空気に幽霊の姿を混ぜた瞬間、全12話の段階的開示という幹が壊れる**。虹色の滲みだけが、まだ名前を持たない何かの気配として残る。

## フォーマット（Format）

**分割しない一枚絵**。近景＝布団の起伏、中景＝伏せずに置かれた画面、遠景＝暗い部屋の奥行きと窓の薄明。時刻（午前二時）・光源（画面）・空気（暗さの中の微光）が全弧の温度を運ぶ。人物は点景以下。既存 imageboard との差は**まさに分割しないこと**——空気は断片にすると空気でなくなる。

## 様式（Style）

ソフトセルアニメ＝柔らかさは光と空気にあり、線にはない。低彩度・少ない色数・光源まわりの穏やかな滲み・空気の薄いもや。この作品の照明条件（一点の弱い光源と広い暗部）にそのまま重なる。
