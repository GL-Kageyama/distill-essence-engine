# 『ハビッツ！！！』 前田亮太 → キャラクター設定画（character-sheet × luminous-anime）

> ## 初稿
> **この設定画は初稿である。変更の可能性が大きい。**
> 出典が与えるのは**年齢・職業・住まい・呼ばれ方・癖・動きの署名・記名・字数**まで。**顔と、体の記述は一行も無い。** 下の「発明（要承認）」の表が、この一枚が足したものの全部である。**承認・差し替えを前提とする。**
> **この一枚には、出典どうしが食い違ったままの欄が一つある**（下の「呼ばれ方の欄の、未決の一点」）。**どちらにも倒していない。**
> シートの隅に、同じ語を**画像の上にも**置く（朱の枠に「初稿」）。

- 入力: [../../../habits-concept-board-luminous-anime/input.md](../../../habits-concept-board-luminous-anime/input.md)（出典の指し先を共有。内容は複製しない）
- format: **キャラクター設定画（character-sheet・登録済み）**——伝達（制作仕様）／一人物 × 多面（時間軸なし）／横長シート
- style: **光の写実アニメ（luminous-anime・登録済み）**
- lang: ja（生成プロンプトは英語）

**この一枚の主題は、この一人の紙と、この一人の手である。** 紙＝**写真のファイル名**（置き配の写真）。**四年分の置き配写真のフォルダ名は、いまも「maeda」である。誰も口にしない。** 手＝**シャッターを一度切る手**——箱を置いたあと、**番地のプレートが画面の左上に来るよう体を回し**、シャッターを切る。**この一枚が置くのは、その体の回し方である。**

## 出典の語（この一人について・一語も変えない）

**版を付けてある。**（run-09＝呼ばれ方の履歴・癖・配属／run-10＝動きの署名）**引用は台帳の文そのままで、欄の名も台帳のままである。** 台帳の参照リンクは、記法を落として語だけを写した。

**この一人の一行（run-09）**

- 男33・配送の先輩・足立区。配送会社の営業所、ドライバー55名、担当は足立区の北半分。六町の2DK、築18年、六町から徒歩9分。通勤は自転車11分で営業所へ、そこから車。妻31（パート）、長女4、長男1。実家は草加市。年収400万台（月31万）。（run-09）

**この一人の呼ばれ方（run-09）**

| 欄 | 出典の語 |
|---|---|
| 職場で | **「前田さん」。** 後輩は「先輩」と呼ぶ。無線では「前田」（run-09） |
| 家族に | 妻が「亮太」、長女が「パパ」（run-09） |
| 誰にも呼ばれない名 | **「前田」。** 四年分の置き配写真のフォルダ名は、いまも**「maeda」**である。**誰も口にしない**（run-09） |

**呼ばれ方の欄の、未決の一点（`⚠ 要確認`）**

- `⚠ 要確認`——**「職場で」の欄に、姓の「前田」が素の形で現れる**（無線では「前田」）。**同じ字が「誰にも呼ばれない名」にも置かれている。**
- **「前田」は素の姓であり、無線でそのまま発音されている。** **《その姓で呼ぶ》に当たらない、と言う余地が無い。**
- **推測でどちらかに倒さない**（`series-bible.md` の改稿履歴に残す）。
- **この一枚の扱い（出典の語ではない）**——**この一枚は、どちらの欄の話も決めない。** 無線の発音も、フォルダ名も、**この一枚では判読できない。**
- **この一人の「呼ばれない名」は、フォルダ名である。** 「maeda」——**四年分の写真が、その文字列の下に重なっている。** そして**誰も口にしない。**

**この一人の癖（run-09）**

- **同じ番地の置き配写真を、四年分、同じフォルダに重ねて撮る。**
- **この一人の癖は、この一人のものではない。**（台帳の地の文が、その持ち主を名で書く。**この一枚は、その名を置かない。**）

**この一人の動きの署名（run-10）**

- **箱を置いたあと、番地のプレートが画面の左上に来るよう体を回し、シャッターを一度切る。**
- **その動作を目に入れている者——**（台帳はその一人を名で書く。**この一枚は、その名を置かない。**）
- **中核四人が「目に入れている者」になる唯一の例である。**

**この一人の記名（run-09／run-10）**

| 欄 | 出典の語 |
|---|---|
| 記名（run-09） | **写真のファイル名。** |
| 記名（run-10） | **置き配の写真。** |
| 動き（run-10 第6話） | 写真を端末から同じフォルダへ、**前田亮太の手で重ねる。四枚目の前で止まる。** |
| 字数（run-09） | **4,200字。** run-10 は4,500字とするが、字数配分は run-09 が決定（`series-bible.md` 三、連載レコード 3） |
| 仮題（run-09） | **「先輩」。** run-10 は「四年分」とするが、仮題は run-09 が決定（`series-bible.md` 三、連載レコード 3） |
| 話名（正） | **「四年分」**（第6話・主役回） |

- 会話（run-10 第6話）——**沈黙が「待ち」に変わる。** **前田亮太は受け取らない。**（台帳の欄は、この会話を四人とする。）

**この一人の扱い（台帳の規律——出典ではない）**

- **主役回は第二巻第6話。** **そして、この一話だけである**——**run-09 は彼に二度目を与えていない。** run-10 が第二巻第12話の会話に彼を出すのは、**会話の層の話であって、立つ話ではない。**
- **記名は写真のファイル名である。** **彼の名は、記名の欄に無い**——**記名の欄そのものが「ファイル名」であり、そこにあるのは姓のローマ字である。**
- **run-10 の注意を、この一人が体現している**——**この世界では、すべての癖は持ち主を持たない。** ある癖が「いい癖だね」と言われるとき、**その癖はもう、その人のものではない。**
- **第6話の会話は四人で、沈黙が『待ち』に変わる。**
- **中核四人が「目に入れている者」になる唯一の例である。**
- **第6話のあとに、彼がどうしていたか**——`—（規律）`。**出典は、主役回を一度置くだけである。** あとを書かない。run-10 の第二巻第12話が彼に与えるのは、**二言だけである。**

**〔導出〕——台帳が導いたもの。出典の語ではない。**
〔導出〕**ずれは、フォルダ名に出る。** 呼ばれる名と、記録に残る名が、**同じ姓である。**
〔導出〕**第二巻の問いに対して、彼は最も遠い位置にいる。** 名は届く。**届いた名を、受け取る者がいない。** そして**癖まで、持ち主を離れている。**

**この一人に無い欄（run-09 / run-10）**

- `動機`・`信念・価値観`・`賭け`・`声`——`?`。**出典に無い。この一枚でも `?` のままにする。** **埋めるなら——**重ねるという撮り方が要る理由（動機）／彼が守っていること（信念・価値観）／この一話で、彼が失い得るもの（賭け）／受け取らないという行為の裏にある、語の選び方（声）。
- `変化弧`・`内面葛藤`——`—（規律）`。一話で立ち、弧を持たないことがこの層の設計である。
- `敵・挑戦の段階`——`—（規律）`。敵役を置かない。

## 発明（要承認）——この一枚が足したもの

| 項目 | 置くもの | 導き方 |
|---|---|---|
| 顔・髪 | 黒髪、短く刈る。色を足さない | 出典に外見の記述が一行も無い（方針 §5a）。**著者の決定（2026-09-16）により、この一枚は人のかたちを置く。** **年齢は出典にある——ゆえに、この一枚の顔はその年齢を示す。これは発明ではない。** |
| 体格の数値 | 置かない。**相対のみ。数値は置かない** | 数は実測として読まれる。**出典に身長の記述は無い** |
| 鞄 | 自転車11分の通勤の、肩掛け | `通勤は自転車11分で営業所へ、そこから車`（run-09） |
| **体の回し方** | 箱を置いたあと、体を回して、**番地が左手の上に来る** | **`番地のプレートが画面の左上に来るよう体を回し`**（run-10）——**この一枚で最も強い発明。** 側面図と背面図に、その回し方が出る |
| 携帯端末 | 撮るための端末を、体の前で構える | **`シャッターを一度切る`**（run-10）——**切るのは一度だけである** |

**置かなかった一つの数。** `四年分`は出典の数である。**この一枚はそれを、注記の一行として置くだけで、写真を四枚描く。** 描くのは、**重なった四年分の写真と、四枚目で止まる手である。**

## 内容（Content）

**②選択＝`受け取らない`。** 核心の問いは「**呼ばれた名は、誰のものか。**」——**第二巻の問いは「届いた名は、誰が受け取るのか」である。** 名は届く。**届いた名を、受け取る者がいない。** 彼の姓は、**四年分の記録の見出しになっていて、誰も口にしない。** そして**癖まで、持ち主を離れている**——**この一人の癖は、この一人のものではない。** **ゆえにシートが固定すべきは、表情の豊かさでも番地でもなく、同じフォルダに重ねる手が、四枚目の前で止まる、その一点である。**

**③翻訳＝フォルダ名を描き、そこに読める名を一つも置かない。** 彼の紙＝**写真のファイル名**、そして**置き配の写真**。**四年分の写真が、その文字列の下に重なっている。** そして**誰も口にしない。** フォルダ名は**判読できない**——**出典では英字である**という一点だけが情報として残る。**写真の中の番地も読めない。**

**④この一枚の中心＝`四枚目の前で止まる`。** 記名の動きから採る。**写真を端末から同じフォルダへ、前田亮太の手で重ねる。四枚目の前で止まる。** 静止画では、**四枚目に手が触れる直前の一点**としてしか描けない——**四つの表情は、すべて「重ねる直前」である。**

**⑤この一枚の四点は、すべて「番地」か「四年分」のどちらかを運んでいる**——**番地のプレート**＝番地、**同じ番地に置かれた箱と、その写真**＝番地、**フォルダの中に重なる四年分の写真**＝四年分、**シャッターを切る指**＝四年分（**四枚目で止まる**）。**この規則に乗らないものは拡大図にしない**——たとえば`ドライバー55名`は出典の数ではあるが、番地も四年分も運ばないので、この一枚では拡大図にしない（出典の語には残る）。

**⑥撮る順は、四つである。** **箱を置く。体を回す。番地が左上に来る。シャッターを一度切る。** **止まるのは、四枚目である。** そして**この一枚は、番地の値をどこにも書かない**——**書けば、同じ番地が、住所になる。**

**⑦癖の持ち主を、この一枚に置かない。** 出典の一行——**この一人の癖は、この一人のものではない。** 目に入れている者は**中核四人の一人である**（台帳はその一人を名で書く）。**ゆえにこの一枚は、見ている者を、この一枚の外に置く**——**四つの表情は、どれも誰にも向かない。** そして**受け取らない**——**名指されても、この手は写真を重ねる。**

**⑧表情4面。** **箱を置く**（run-10）／**番地のプレートが画面の左上に来るよう体を回す**（run-10）／**シャッターを一度切る**（run-10）／**受け取らない**——**名指されても、写真は同じフォルダへ重なる**（run-10 第6話の会話）。**四つとも「重ねる直前」で止める。**

**⑨ガード。** フォルダ名は**判読できない**（**出典では英字である**）。**番地のプレートも判読不能**である。**彼の姓は、この一枚のどこにも呼ばれない**——**呼ばれ方の欄の食い違いを、この一枚で決めない。** ゆえに Negative に`no legible file name`を、`no legible address`を、`no romaji in place of the Japanese name`を置く（**ヘッダの名は日本語のまま**である）。**恐怖もかわいいも出さない。**

## フォーマット（Format）

白地・多面図4面（正面／斜め／側面／背面）を**同一の身長線と目線**に載せる。中立のAポーズ、腕はシルエットから離し、**手は空**。下段に表情4面（上の四つ）。右に拡大図四点——**番地のプレート**、**同じ番地に置かれた箱と、その写真**、**フォルダの中に重なる四年分の写真**、**シャッターを切る指**。隅に朱の枠で**「初稿」**。**描かれる文字は三種**——ヘッダの名（**日本語**）、引き出し線の注記（**英語**）、そして「初稿」（日本語）。**作中の文字（フォルダ名、番地のプレート）は判読不能のまま**置く。

**身長は相対のみ。数値は置かない。**

## 様式（Style）

光の写実アニメの**生き残る部分だけ**を使う。**清潔なアニメ線画**と**配色の規律**——曇った昼の白、深いシアンに寄る影、そして**一箇所だけ最も強いハイライトを受ける点。**

> **様式メモ（この様式が設定画で失うもの）。** この様式の主役は光であり、設定画の主役は人物である。ゆえに**四つの忠実性アンカー（層を成す空・体積光・フレアとブルーム・夕景の配色）は、この一枚ではすべて落ちる。** 落とさなければならない——白地に空を置けば、それは format の avoid「人物の背後の情景」であり、**雰囲気が情報に勝つ。** 残るのは線と配色の規律だけで、それで足りる。**朱の「初稿」枠だけが、この作品の朱であり、同時にこの様式の「最も強いハイライトを受ける一点」である。**
> **この一枚には、様式が与えるはずの「最も明るい一点」が人物の上に一つも無い。** 出典は彼を`配送の先輩`と書き、担当を`足立区の北半分`と書く——**最も平凡な光で、いちばん多くを記録に残す人物。それが彼の仕様である。** レンズは、この一枚では**向けられるが、光らない。**

## 合成プロンプト（Merged）

```text
A character model sheet of 前田亮太 — a 33-year-old senior delivery driver in Adachi, Tokyo, one of 55 drivers at a depot, covering the northern half of the ward, who stacks four years of photographs of the same addresses into one folder and does not take the habit back when it is named — on a plain white ground, four views in one row — front, three-quarter, side and back — all at the same character height and the same eye line on a shared horizontal guide, in a neutral A-pose with arms clear of the silhouette, both hands empty. In the sheet's corner, small, a bordered box outlined in vermilion holding the two Japanese characters 初稿; beside it the name in Japanese characters, set small, exactly as the work writes it, with no reading glossed above it.

The age stated above is a fact of this sheet: every face on it carries that age — the four views and the four expression heads alike, at every size. It is not a default young face.

He is precisely specified: a 2DK in Rokucho, 18 years old as a building, nine minutes on foot from the station and eleven minutes by bicycle to the depot before he gets in the car; short-cropped black hair; a shoulder bag; a handset held out in front of him. After the box is set down he turns his body until the address plate comes to the upper left of the frame, and he shoots once — one shutter only.

The whole surface of this sheet is a file name, and four years of photographs are stacked beneath it while nobody says the word. Enlarged, four insets: the address plate; a box left at the same address with its photograph; four years of photographs stacked inside the one folder, the folder name nowhere legible; and the finger closing the shutter, stopped one beat short of the fourth. Nothing carries what these four do not. He does not write the address down anywhere in the frame.

Below, a row of four expression heads, each held one beat short of stacking the fourth: setting the box down; turning the body until the address plate sits upper left; closing the shutter once; and not taking it — the habit named to him and the photographs going into the same folder anyway.

Flat even light, the shading held inside the figure only, no cast shadow on the ground, no background scene, callout annotations on leader lines throughout, in English only, clean anime lineart, overcast daylight white with a cyan-leaning shadow, one saturated note only — the vermilion of the 初稿 box. A production reference readable as a specification, not a finished illustration. The header name in Japanese is the only legible name in the frame; the folder name and the address plate are nowhere legible, and nobody in the frame is looking at him.

Not photorealistic, no 3D render, no muted desaturated palette, no flat gradient sky, no grain, no painterly brush strokes, no photographic faces, no generic young face, no face younger than the age stated, no idealized anime default face, no face in the four views younger than in the expression heads, no sky, no god rays, no lens flare, no light particles, no dramatic lighting, no background, no garbled text or mojibake, no legible name on any in-world prop, no romaji in place of the Japanese name, no legible file name, no legible address, no legible numerals, no onlooker, no street, no second shot.
```
