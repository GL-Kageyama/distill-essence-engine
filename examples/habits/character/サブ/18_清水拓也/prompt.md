# 『ハビッツ！！！』 清水拓也 → キャラクター設定画（character-sheet × luminous-anime）

> ## 初稿
> **この設定画は初稿である。変更の可能性が大きい。**
> 出典が与えるのは**年齢・職業・勤め先・住まい・呼ばれ方・癖・動きの署名・記名**まで。**顔と、髪と、体格の数値だけが与えられていない。** 下の「発明（要承認）」の表が、この一枚が足したものの全部である。**承認・差し替えを前提とする。**
> シートの隅に、同じ語を**画像の上にも**置く（朱の枠に「初稿」）。
>
> **この一枚は、引き渡された二版（run-09 / run-10）だけを材料にする。** 出典の各行に版を付けてある——著者が行ごとに可否を決められるように。

- 入力: [../../../habits-concept-board-luminous-anime/input.md](../../../habits-concept-board-luminous-anime/input.md)（出典の指し先を共有。内容は複製しない）
- format: **キャラクター設定画（character-sheet・登録済み）**——伝達（制作仕様）／一人物 × 多面（時間軸なし）／横長シート
- style: **光の写実アニメ（luminous-anime・登録済み）**
- lang: ja（生成プロンプトは英語）

**この一枚は、一つの紙と、一つの手のためにある。** 清水拓也の記名の紙は**送り状の控え**であり、彼の動きの署名は**片手で画面を四角に合わせて、一度だけ押す**ことである。**この二つが、この一枚の主題である。** 紙に書かれているのは送り主と届け先の名であり、押すのは彼の指である。**その二つが、まだ開けられていない箱を挟んでいる。**

## 出典の語（この一人について・一語も変えない）

**版を付けてある。**

**この一人の輪郭（run-09）**

- `男30・別会社の配達・江戸川区。`／`配送会社の営業所、ドライバー40名、担当は江戸川区の東半分。`／`西一之江の1K、築22年、一之江から徒歩10分。`／`通勤は自転車8分で営業所へ、そこから車。`／`妻29（パート）、長男3。実家は市川市。`／`年収400万台（月27万）。`

**この一人の呼ばれ方（run-09）**

- 職場で——`無線では`／`「清水、了解」。`／`伝票には姓だけ。`／`届け先では会社の名で呼ばれる`
- 家族に——`妻が「拓也」、長男が「パパ」`
- 誰にも呼ばれない名——`「拓也」。`／`妻は三年目のいま、「パパ」しか言わない。最後にその音を出したのは、長男が生まれる前の晩である`

**この一人の癖と、動きの署名（run-09 / run-10）**

- 癖（run-09）——`翌朝、開ける前の箱の写真を一枚だけ撮る。`
- 動きの署名（run-10）——`箱を地面に置いたまま半歩引き、片手で画面を四角に合わせて一度だけ押す。`
- その動作を目に入れている者——`その動作を目に入れている者——同じ番地の住人。`（run-10）
- `片手で、四角に合わせて、一度だけ。`／`三つの「ひとつ」が重なっている。`

**この一人の記名と、その回の動き（run-09 / run-10）**

- 記名（run-09）——`送り状の控え。`
- 記名（run-10）——`開ける前の箱。`
- 動き（run-10 第9話）——`箱の写真を画面から画面へ、`／`清水拓也の手で撮る。一枚で止まる。`
- 会話（run-10 第9話）——`清水拓也と住人。`／`二言だけ。`
- 字数（run-09）——`4,000字。`
- 仮題（run-09）——`「箱の写真」。`
- 話名（正）——`「一枚だけ」`（第9話・主役回）

**この一人の配置（run-09 / run-10）**

- **配属** 第二巻『重なった名』第9話・主役回。**二度目を持たない。**
- `この動作を目に入れている者は、同じ番地の住人である`（run-10）。／`住人は、二言だけ交わす。`
- `「長男が生まれる前の晩」を、本文で説明しない。`／`出典は日付を書かない。`

**この一人の位置（台帳の〔導出〕の節より・〔導出〕と明記して引く）**

- 〔導出〕核心の問いと、この巻の問い——`呼ばれた名は、誰のものか。`／`第二巻の問いは「届いた名は、誰が受け取るのか」である。`
- 〔導出〕欲求——`開けられる前の状態を、一枚だけ残すこと。`
- 〔導出〕`能動の原理`——`開けられる前の状態を、一枚だけ残すために動く。`
- 〔導出〕`ずれは、呼び方が三つとも「名でない」ことに出る。`／`無線は姓、届け先は法人名、家は「パパ」。`／`三つの場所が、三つとも名を出さない。`

**この一人に無い欄（run-09 / run-10）**

- `動機`・`信念・価値観`・`賭け`・`声`——`?`。**出典に無い。この一枚でも `?` のままにする。** **埋めるなら——**一枚で止める理由（動機）／開ける前を残すことが守っているもの（信念・価値観）／この一話で失い得るもの（賭け）／二言という数の裏にある語の選び方（声）。
- `変化弧`・`内面葛藤`・`敵`・`敵の抵抗`——`—（規律）`。**一話で立ち、弧を持たないことがこの層の設計である。**
- **「別会社」がどこか**——`—（規律）`。**出典は「別会社の配達」とだけ書く。会社の名を書かない。**

## 発明（要承認）——この一枚が足したもの

| 項目 | 置くもの | 導き方 |
|---|---|---|
| 顔・髪 | 黒髪、短く、帽子の跡がつく長さ。**この一枚の同一性は、髪から読まれない** | 出典に外見の記述が一行も無い（方針 §5a）。**著者の決定（2026-09-16）により、この一枚は人のかたちを置く。** **年齢は出典にある——ゆえに、この一枚の顔はその年齢を示す。これは発明ではない。** |
| 体格の数値 | 置かない。相対の基準も置かない | 出典に体格の記述が一行も無い。**数は実測として読まれる。** |
| 衣装 | 配送の作業着。**この一枚の同一性は、衣装から読まれない** | 職業＝配達（run-09）。出典に衣装の記述は一行も無い |
| 鞄 | 持たない。自転車の籠に置く | `通勤は自転車8分で営業所へ、そこから車。`（run-09） |
| 片手に持つ画面 | 小さな画面を片手で持つ。**撮る道具は、この一枚に一つだけ** | `片手で画面を四角に合わせて一度だけ押す。`（run-10） |
| **封の切られていない箱** | **ガムテープが、まだ切られていない。** 中身は、まだ誰のものでもない | 記名（run-10）＝`開ける前の箱。`＋癖＝`翌朝、開ける前の箱の写真を一枚だけ撮る。`（run-09）。**この一枚で最も強い発明** |

**この一枚で最も強い発明は、切られていない封である。** 人物の造形——髪も、衣装も、体格も——はこの一枚の主題ではない。**主題は、開けられる前の状態が、まだ壊れていないことである。**

## 内容（Content）

**②選択＝`開ける前の箱。`** この作品の中心語は名である——`名は呼ばれた回数でできている`。**第二巻の問いは「届いた名は、誰が受け取るのか」である。** 清水拓也は、その問いに対して**「受け取られる前」という位置を足す**（〔導出〕）——**彼だけが、開けられる前で止まっている。** そして**彼自身の名は、いちばん近い者から出た最後が、子供が生まれる前である。** **ゆえにシートが固定すべきは、箱が閉じたままであることである。**

**③翻訳＝その紙を、まだ切られていないまま描く。** 記名は run-09 が`送り状の控え。`、run-10 が`開ける前の箱。`とする。**二つとも置く**——**送り状の控えは、送り主と届け先の名を書く紙であり、開ける前の箱は、その名がまだ受け取られていない物である。** **どちらも判読不能とする。** そして**彼の名は、無線と伝票と家の、どこでも出ない**——`三つの場所が、三つとも名を出さない。`

**④この一枚の中心＝`一枚で止まる。`** run-10 の一行——`清水拓也の手で撮る。一枚で止まる。` **`片手で、四角に合わせて、一度だけ。`** `三つの「ひとつ」が重なっている。` **ゆえにこの一拍は、二度目が来ないことによってできている。** 静止画では、**指が画面の上にあり、まだ離れていない一点**としてしか描けない——**四つの表情は、すべてそこで止まる。**

**⑤拡大図四点が運ぶもの。** 初稿の規則を踏襲する——**四つとも「年」か「名」のどちらかを運ぶ。** 封の切られていない箱＝**名**（まだ受け取られていない名）。送り状の控えの、送り主と届け先の欄＝**名**（判読不能）。画面に合わせた四角＝**名**（その四角の中にしか、まだ名が無い）。一枚だけ撮られた、その一枚＝**年**（開けられる前の、最後の状態）。

**⑥この一枚に、二枚目は無い。** `一枚だけ`という値は、**時間の上では「一度で止めた」である。** 静止画に置けるのは、**二枚目がどこにも無いこと**だけである。**ゆえにこの一枚は、画面も、控えも、箱も、それぞれ一つしか置かない。** **数を足せば、この人物の値は消える。**

**⑦三つの「ひとつ」のうち、図に置けるのは二つである。** `片手で、四角に合わせて、一度だけ。`——**「片手」は形に置ける。「四角に合わせて」は、画面に置く四角として置ける。「一度だけ」は置けない。** ゆえに**三つ目は、引き出し線の注記で名指す。** 図を増やすのではなく、**注記の側に一つを預ける。**

**⑧表情4面。** 翌朝、開ける前の箱の写真を一枚だけ撮る（run-09）／箱を地面に置いたまま半歩引き、片手で画面を四角に合わせる（run-10）／**一度だけ押す——一枚で止まる**（run-10 第9話）／同じ番地の住人と、二言だけ交わす（run-10 第9話）。**四つとも出典から採り、四つとも、指が画面の上にある一点で止める。**

**⑨ガード。** **この一枚に `「拓也」` を置かない**——呼ばれない名は、描かれない。**送り状の控えの名も判読不能**である。**読みを振らない。ローマ字に置き換えない。** **日付を書かない**——`「長男が生まれる前の晩」を、本文で説明しない。` **会社の名も書かない**（`—（規律）`）。**二言の中身を発明しない。** 恐怖もかわいいも出さない。**開いた箱を置かない。**

**記録として残す一つの事実。** 出典は、彼の呼び方を三つ並べ、**三つとも名でない**と書く。**それでも、彼は毎朝、まだ名のついていない箱を一枚だけ撮る。** **この一枚のヘッダの名は、呼ばれない側の名から来ている。**

## フォーマット（Format）

白地・多面図4面（正面／斜め／側面／背面）を**同一の身長線と目線**に載せる。中立のAポーズ、腕はシルエットから離し、**手は空**。下段に表情4面（上の四つ）。右に拡大図四点——**封の切られていない箱**、**送り状の控えの、送り主と届け先の欄**、**画面に合わせた四角**、**一枚だけ撮られた、その一枚**。隅に朱の枠で**「初稿」**。**描かれる文字は三種**——ヘッダの名（**日本語**）、引き出し線の注記（英語）、そして「初稿」（日本語）。**作中の日本語（送り状の送り主と届け先、伝票）は判読不能のまま**置く。

**身長は相対のみ。数値を置かない。** **この一枚は、順位を発明しない**——出典に体格の記述が一行も無いためである。**四つの拡大図は、すべて「年」か「名」のどちらかを運んでいる。この規則に乗らないものは拡大図にしない。**

## 様式（Style）

luminous-anime の**生き残る部分だけ**を使う。**清潔なアニメ線画**と**配色の規律**——早朝の曇った白、深いシアンに寄る影、そして**一箇所だけ最も強いハイライトを受ける点。**

> **様式メモ（この様式が設定画で失うもの）。** **この様式の主役は光であり、設定画の主役は人物である。** ゆえに四つの忠実性アンカー（層を成す空・体積光・フレアとブルーム・夕景の配色）は、**この一枚ではすべて落ちる。落とさなければならない**——白地に空を置けば、それは format の avoid「人物の背後の情景」であり、**雰囲気が情報に勝つ。** 残るのは線と配色の規律だけで、それで足りる。**朱の「初稿」枠だけが、この作品の朱であり、同時にこの様式の「最も強いハイライトを受ける一点」である。**
> **この一枚には、様式が与えるはずの「最も明るい一点」が、人物の上に一つも無い。** 出典が与える最も強い光は、**箱の上に貼られた送り状の、白い面である。** **そこには判読不能の名が書かれている。** **この様式の「最も明るい一点」が、この一枚では、読めない名の上に落ちる。**

## 合成プロンプト（Merged）

```text
A character model sheet of 清水拓也 — a 30-year-old delivery driver working for another company in eastern Edogawa, Tokyo, who photographs a parcel the morning after, one frame only, before anyone has opened it — on a plain white ground, four views in one row — front, three-quarter, side and back — all at the same character height and the same eye line on a shared horizontal guide, in a neutral A-pose with arms clear of the silhouette, both hands empty. In the sheet's corner, small, a bordered box outlined in vermilion holding the two Japanese characters 初稿; beside it the name in Japanese characters, set small, exactly as the work writes it, with no reading glossed above it.

The age stated above is a fact of this sheet: every face on it carries that age — the four views and the four expression heads alike, at every size. It is not a default young face.

He is 30, a delivery driver, and the sheet gives what the source gives and nothing more: black hair, cut short enough to sit under a cap; the plain work clothes of a delivery depot; no bag, because he rides eight minutes to the depot and leaves everything in the cab. Neither the hair nor the clothes are how this sheet is told apart — it is told apart by one slip of paper, one parcel and one hand.

His identification is two things at once: the sender's copy of a shipping slip, holding the name of whoever sent the parcel and the name of whoever will receive it, both in handwriting and both nowhere legible; and the parcel itself, still sealed, its tape uncut, because nothing in it belongs to anyone yet. His own name appears in none of the frame's papers — not on the slip, not on the delivery note.

In this frame there is exactly one of everything. One parcel, one shipping slip, one small screen held in one hand. Enlarged, four insets, each carrying either a year or a name: the sealed parcel with its tape uncut; the sender and recipient fields on the shipping slip, nowhere legible; the rectangular frame brought level around the parcel on the screen, slightly wider than the parcel itself; and the single frame that was taken, held as a still, the last state before the box was opened.

Below, a row of four expression heads, each stopped with a finger still on the screen and not yet lifted: taking one photograph of a parcel nobody has opened yet; stepping back half a pace with the parcel left on the ground and levelling the frame with one hand; pressing once, and stopping at one; and exchanging two words with a neighbour at the same address.

Flat even light, the shading held inside the figure only, no cast shadow on the ground, no background scene, callout annotations on leader lines throughout, in English only, clean anime lineart, cool early-morning white with a cyan-leaning shadow, one saturated note only — the vermilion of the 初稿 box. A production reference readable as a specification, not a finished illustration. The header name in Japanese is the only legible name in the frame; every name on an in-world prop is nowhere legible. The three ones — one hand, one frame, one press — are named on a leader line; only one photograph exists in the frame.

Not photorealistic, no 3D render, no muted desaturated palette, no flat gradient sky, no grain, no painterly brush strokes, no photographic faces, no generic young face, no face younger than the age stated, no idealized anime default face, no face in the four views younger than in the expression heads, no sky, no god rays, no lens flare, no light particles, no dramatic lighting, no background, no garbled text or mojibake, no legible name on any in-world prop, no romaji in place of the Japanese name, no legible sender name, no legible recipient name, no legible text on the shipping slip, no company name or logo, no opened parcel, no cut tape, no second photograph, no image gallery, no date written anywhere, no handwriting by the subject, no signature, no red seal impression.
```
