...
**「内容の本質を、イメージへ圧縮すること」** —— これがこのエンジンのすることの全部。そのイメージが何であるかは、フォーマットが決める。

入力（時間×意味×要素の無限情報）を、**一回の生成の容量**という制約に、本質を保ったまま畳み込む。その容量を宣言するのはフォーマットカードである——時間軸を持たない一枚（サムネイル・表紙・挿絵・アイコン）、複数枚（漫画・ストーリーボード・絵本）、あるいは固定の持続（動画仕様——16:9 の三十秒）。

**2D 静止画像はエンジンの終着点ではない。** それは容量に時間軸がない場合であり、歴史的には最初の場合で、いまも最も多い場合だが、いくつかある場合の一つにすぎない。フォーマットとは、その容量の**内側**での戦略の差である——「粒度」と「配置」、そして容量が時間軸を持つときには「時間あたりの密度」。
...
- **一点**：ある瞬間だけ（イラスト）
- **時間のまま**：容量そのものが時間軸を持つので、時間は畳まれない——配分される（動画仕様：固定の持続にわたるビート表）
