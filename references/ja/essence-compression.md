...
入力（時間×意味×要素の無限情報）を、**一回の生成の容量**という制約に、本質を保ったまま畳み込む。その容量を宣言するのはフォーマットカードである——時間軸を持たない一枚（サムネイル・表紙・挿絵・アイコン）、複数枚（漫画・ストーリーボード・絵本）、あるいは固定の持続。
...
- **一点**：ある瞬間だけ（イラスト）
- **時間のまま**：容量そのものが時間軸を持つので、時間は畳まれない——配分される（固定の持続にわたるビート表）
