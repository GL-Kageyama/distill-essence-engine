...
- One scene per board
- Give each character's distinguishing appearance — gender, hair, build, clothing — once, in square brackets as a hidden note the model reads but does not draw (`[Name: gender, hair, build, clothing]`), so the model places the right figures; derive it from the source (⑧)
