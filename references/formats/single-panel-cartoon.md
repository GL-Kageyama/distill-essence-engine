...
- Give the character's distinguishing appearance — gender, hair, build, clothing — once, in square brackets as a hidden note the model reads but does not draw (`[Name: gender, hair, build, clothing]`), so the model draws the right character; derive it from the source (⑧)
