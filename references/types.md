<!-- i18n-version: 1.0.0 | canonical: references/types.md | translated: 2026-08-27 -->
...
| Others | **Design** | Give a brand, product, or app a visual identity | Logo / icon / poster / app screen / brand board |

The three stages are **assumed purpose → the format's function → granularity × time**. The assumed purpose decides the direction of the function (understanding/communication → explanatory type, attraction → attention type, re-experience → symbolic/narrative type, record → narrative type, decoration → decorative type, design → symbolic (marks) / layout (screens & systems)). This is the precise form of "purpose is upstream of compression".
...
| **Production reference** | Character model sheet / character image board / concept board / art board / location board / key pose board / scene board / progress board | character-sheet: one character × multiple views (no time axis) ／ character-board: one character × folding (exploration) ／ concept-board: whole arc × folding (one undivided image) ／ art-board: one location × a single point ／ location-board: the story's places × folding (geography on one board) ／ key-pose-board: one subject × its movement vocabulary (key poses) ／ scene-board: one scene × a single point ／ progress-board: the whole of a production × no time axis (one undivided image) |
| **Layout** | App screen / landing page / wireframe / brand board / business card | app-screen: one screen × a single point ／ landing-page: one page above the fold × a single point ／ wireframe: one screen skeleton × a single point ／ brand-board: the whole identity system × folding (one board) ／ business-card: one card × a single point |

The combination of 7 purposes × granularity/time, plus the production-reference family (documents for a later stage to reproduce from, not pictures to enjoy) and the design family (layouts for a brand, product, or app — the input is a brand / product / app, not content). The space itself is infinite, and these 11 types are nothing but **crystallized named formats** (＝ representative points).

**Size and aspect ratio are also decided by the format** (decided by where it is seen): thumbnail = small, landscape 16:9; icon = square; cover = portrait (book ratio); poster = large, portrait; storyboard = cinema ratio 16:9; manga = page ratio; infographic = tall scroll; timeline = wide band (or vertical scroll); flowchart = vertical; comparison matrix = wide grid; data chart = wide, single panel; conceptual illustration = a single image, flexible ratio (square to wide). The manga family adds its own: four-panel = a tall fixed 4-panel column; webtoon = a long single vertical scroll canvas (mobile); comic strip = a wide horizontal one-row strip (newspaper); splash page = a full-bleed single panel (double-page spread); single-panel cartoon = a compact single panel; comic essay / reportage manga / educational manga / episode board = manga page ratio (episode board: one self-contained page). The production-reference family is decided by what a later stage reads from: character model sheet = a wide sheet, a row of views on a plain ground; character image board = multiple panels on one board; concept board = a single image, cinematic to flexible ratio; art board = one background plate, 16:9; location board = multiple panels on one board; key pose board = a row of key poses on one board; scene board = one full scene plate, 16:9; progress board = a single image, 16:9. The design family is decided by where the artifact is seen: app screen = the device ratio (portrait mobile / landscape desktop); landing page = desktop 16:9 above the fold; wireframe = the screen's ratio, grayscale; brand board = one wide board; business card = a small standard card (two sides).
...
- **Design lineage family** (a lineage cross-cut of the typographic / digital medium): [swiss-style](styles/swiss-style.md) / [flat-ui](styles/flat-ui.md) / [material-ui](styles/material-ui.md) / [neubrutalism](styles/neubrutalism.md) — the four named design dialects share one DNA (a designed artifact's grammar, the layout as a visible system, restraint + one accent) and differ in the dialect of the design movement. Added and verified 0.1.30 (design-* format-swap and style-swap cases; judgments in [examples/EVALUATION.md](../examples/EVALUATION.md)).
