...
| **Production reference** | Character model sheet / character image board / concept board / art board / location board / key pose board / scene board | character-sheet: one character × multiple views (no time axis) ／ character-board: one character × folding (exploration) ／ concept-board: whole arc × folding (one undivided image) ／ art-board: one location × a single point ／ location-board: the story's places × folding (geography on one board) ／ key-pose-board: one subject × its movement vocabulary (key poses) ／ scene-board: one scene × a single point |
