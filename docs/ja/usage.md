...
エンジンは英語の画像プロンプトを 3 欄（Content／Format／Style）＋合成プロンプトで返す。そのまま Stable Diffusion / Midjourney など に貼れる。
...
| `lang` | en / ja / zh — 説明・トレースの言語（画像プロンプト自体は常に英語） |
...
英語の画像プロンプトを 3 欄（内容／フォーマット／様式）＋末尾の合成プロンプトで出力。SD/MJ などにそのまま渡せる。
