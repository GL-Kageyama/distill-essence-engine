...
引擎会返回英文提示词，分三栏（Content／Format／Style）＋合成提示词，可直接粘贴到 Stable Diffusion / Midjourney 等——若格式带时间轴，则返回可粘贴到 Wan 等的填好的规格书。
...
| `lang` | en / ja / zh — 说明与 trace 的语言（生成的提示词本身始终是英文） |
...
以三栏（Content / Format / Style）输出英文提示词，并在末尾追加合成提示词。可直接粘贴到 SD/MJ 等（格式带时间轴时，以填好的规格书代替合成提示词）。
