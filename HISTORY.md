...
- **著者の指示により、外見デザインの実測を形式カードへ入れた。**「外見デザインのエッセンスを学んでいるのだから、その学びを残せ」「git のどこかに入れよ」——**置き場所は `references/formats/character-sheet.md` である**（実測はこの形式の上で取り、このカードは後工程が顔を書くときに読む一枚である）。**×3言語に `## Specifying a face`（顔の指定）を追加した。**
- **出所は、この樹の `examples/habits/character/` の設定画 28 枚の改訂である。****中心は `01_佐藤美咲`（第五稿）と `13_田村彩`（第四稿）の二枚**——**同じ語彙で四度・五度書き直し、一度だけ通った。****⚠ 実測の生の記録は私的な設計ノートに在り、この樹には置かない**（配布物はこの樹のみ）。**この樹が持つのは結論だけである。**
- **入れた七つの法。**
    - **① 形は大きさを要し、地は大きさを要さない。****地＝比率と骨**（上下の比・頬骨・顎の角・目の間隔・鼻・耳）は**どの大きさでも読む——表情4面の小さい頭でも揃った。****形＝微細な部位**（下まぶた・目の下の影・唇の端の削れ）は**拡大図の大きさでのみ読む**——**頭の大きさで名指した四度は、一度も読まれなかった。**
    - **② 既定は中立ではない。それは様式の平均という、一つの特定の顔である。****指定しなければ、それを選んだことになる。****ゆえに否定形は地を指定しない**（`面長でも丸顔でもない` は欄を空のままにする）。**効いたのは、地を肯定形で名指すことである。**
    - **③ ⚠ 引き出し線が引かれたことは、値が動いたことを意味しない。****様式は軸ごとに「既定の側の値」を持つ**（この実測で見えた既定＝額が広く目が下寄り／頬骨が平ら／顎が丸い／目が離れている／鼻が短い／耳が小さい）。**ゆえに軸を既定側の値で書くことは、指定ではなく、もう一度の既定である。****⚠ これは未確定である**——**私は一度「六つとも乗った」と数え、のちにそれが数えすぎだった可能性に気づいた。**
    - **④ ⚠ 枠は残りやすく、中身は落ちる。****人物の背後の情景は指示どおり落ちた。****⚠ ただし五枚に一枚、指示を一字も変えていないのに拡大図の枠が落ちた。****ゆえに「落ちない」ではなく「落ちないことが多い」と書いた。**
    - **⑤ 数を禁じると、文字になる。****数を禁じれば判読できる数が別の場所に現れた**（`20年` が `2026` になった）。**そして、数は数えたとおりに書く**——**多面図と表情4面を名指して拡大図を名指さない否定形は、誰も求めていない顔の拡大図をモデルに足させた**（四点が八点になった）。
    - **⑥ 地と形を同じ改訂で動かすと、測れない。****両方が同時に動いていると、どちらを読んでいるのか分からなくなり、その改訂が答えようとしていた問いが未回答のまま残る。**
    - **⑦ 向きは顔に乗る。齢は数では乗らない。****俯きは俯きとして読む。****齢を書いただけでは、顔が既定の若い顔のままであることを止められない**——**Negative に `no generic young face` を三度置いても止まらなかった。**
- **`do` と `avoid` に各一行を足した**（地を肯定形で名指し、どの軸も様式の既定に任せない／顔を未指定のまま残さない、微細な部位が頭の大きさで読まれると期待しない）。
- **⚠ 実測の範囲を、カード本文に明記した。****すべてこの形式・一つのアニメ様式（`luminous-anime`）の上で取った。****別の様式で、あるいは時間軸を持つ形式で、地と形が同じに振る舞うかは測っていない**——**ゆえに法として書き、範囲をその隣に置いた。**
- **英語不変部は一字も動かしていない**——プロンプトテンプレと Negative は三言語でバイト一致のままである（検査で確認）。**動かしたのは説明の散文と do/avoid だけである。** ミラー三枚の `translated:` を 2026-09-17 へ更新した。
- SKILL.md の version 0.1.47→0.1.48。

## 0.1.47 — 2026-09-17

- **新フォーマット `progress-board`（進捗盤）を登録した（著者の承認・「凍結して登録する」）。** 用途は **象徴（制作仕様）／制作の全量 × 時間軸なし（分割しない一枚）**。**主題は制作の中身ではなく制作の状態**であり、**数値を一枚も入れずに**「どこまで進んだか」を表す。**制作仕様ファミリーの 8 枚目**。
- **物差しは外から借りない。** カードの構成文法の中核は「**物差しは作品から取る**」——作品が既に仕上がりの語彙（印の解像度・材の品位・摩耗の度合い）を持っているなら、**その語彙が物差しである**。百分率・バー・信号・計器・目盛り板を禁じ、**量は一つ一つの物がどう作られているかから読み手が回復する**。**まだ作られていない側には担い手が要る**——存在しない物は摩耗せず、無い物は痕跡を残さないので、ゼロの端を**面の状態**（手が渡っていない場所・仕上げがまだ全体で木目がまだ木にある）が運ぶ。**⚠️ 発光する輪郭・空の額縁・半透明のシートは禁じた**——紋切型の隠喩であり、負の一覧が既に禁じている額縁とガラスの語彙と正面から衝突する。
- **検証（`examples/habits-status/`＝『ハビッツ！！！』の制作状態。実在の作品ケース）**：**第三枚「目録」が閾値の両方を満たした**——**anti-generic 84／genericness 2/10／計上シグネチャ 0件**、**美的 74**（≥7/10）。**⚠️ 姉妹二枚（第一枚「全量」・第二枚「現在地」）は anti-generic 66 で FAIL のままである**（美的は 75 で PASS）。主因は実測で名指しされた二点——接写が俯瞰をほぼ言い直していること（39語の逐語一致）と、`fine dust drifting` の残存。**著者が「これはこれでいい」と述べてこの二枚をそのまま置いたため、登録の根拠は第三枚の PASS に置いた。** 判定・推移・残件は [`examples/EVALUATION.md`](examples/EVALUATION.md) の 0.1.47 の項に全量を置いた。
- **⚠️ この検証で、この樹の既知の二つの壊れ方が再発した。** ①美的判定の第1回が `tool_uses: 0`・`｜｜DSML｜｜` の疑似ツール呼び出しで崩れ、**`general-purpose` が定義ファイルを読んで代替判定**した（0.1.28 と同手順・EVALUATION に明記）。②**紋切型シグネチャの計上が周回で反転した**——同一系統の判定者が `its fall is broad and flat` を、ある周回では「共有の光源リグの記述であり移植可能な内容句ではない」として除外し、次の周回では「31字の一致」として計上した。**点数は 70→76→74→77→82→84 と動き続け、シグネチャ条件だけが FAIL を出し続けた。** **ゆえに残件一件を「あと一周で通る」と読まない。**
- **登録と索引**：`formats/progress-board.md` ×3言語（ja・zh はミラー。**プロンプトテンプレは三言語でバイト一致**＝英語不変部）。`registry.md` ×3言語にフォーマット 1 行。`types.md` ×3言語の制作仕様ファミリー（一覧・粒度×時間・サイズ段落）に追記。README ×3言語の目的別フォーマット表に `progress board` を追加。**件数を持つ全箇所を実数へ是正**——**フォーマット 43→44**（README ×3言語の2箇所ずつ・`docs/index.md` ×3言語・`examples/README.md` ×3言語）。**⚠️ 真は「registry の行数」であり、ディスク上のファイル数ではない。** `examples/habits-status/` の三枚と `input.md` の「未登録」の記述も、登録済みへ更新した（**英文の Merged は一字も動かしていない**——動かせば第三枚の PASS が無効になる）。
- **⚠️ 未登録のカードがディスク上に 1 枚在ることを実測した（報告のみ・未手当て）**：`references/formats/episode-board-v2.md`。**登録済みの `episode-board` とは別ファイルで、registry ×3言語のどこからも参照されていない。** ゆえに 0.1.47 の時点で**登録 44 行・ファイル 45 枚**。**私が作ったものではない**（登録前から在り、当時の実数は登録 43・ファイル 44）。**登録するか捨てるかは著者の判断。**
- SKILL.md の version 0.1.46→0.1.47。

## 0.1.46 — 2026-09-16

- **著者が生成四点を承認し、この樹に入れた。****あわせて、置き換えられた一点（`01` の 04:34:05）を外した**——**`8f792fa` に在り、履歴から戻せる。****画像を入れるのは、この樹で初めてである。**
- **⚠ 四点はすべて、0.1.45 の `(j)(k)(l)` を含まない版からの出力である**（`prompt.md` の更新は 04:43:16）。**ゆえに、これで測れるのは 0.1.42 の `(h)(i)` の側だけである。**
- **`02_鈴木和彦`（04:49:46）と `04_片山大翔`（04:51:03）を測った**——**この二枚が、`(h)(i)` の三枚目・四枚目である。**
    - **✅ `(i)` は効いた**——**四枚とも、`Age`／`Occupation` の塊が無い。****作中の名も判読不能である**（`02` の名札は白、乗車記録の名の欄は空、点呼簿は番号だけ。`04` の記簿の罫線は空、自分の名の紙も空）。**`04` は四枚とも顔で、ビートが手と顔の両方に在る——この樹で最も良い一枚である。**
    - **⚠ ただし、`caption` の診断は弱くなった。****同じ条項の下で、`01` は注記が四点すべて落ち、`02` は四点すべて残った。****ゆえに「注記が落ちたのは `caption` の一語のせい」は推測より弱い。** `(k)` が注記を救うかは、**生成してみるまで分からない。**
    - **⚠ 多面図が表情4面より若いという残欠陥は、`02` でも出た**——**三枚目である**（前の版は二十八〜三十二。**私の読みで、多面図は三十代、表情4面は四十〜五十代**）。
- **✅ 語の水準の実測と、画像が四枚とも一致した。** 「表情4面の段落に顔の部位を名指す語を持つか」で分けたところ——**`01`・`02`・`04` は語を持ち、四つとも顔。****`03` は語を持たず、四つとも手。****これは `(l)` の根拠が、画像の側から一本立ったということである。****⚠ ただし n=4 である**——**残る二十三枚の画像は一枚も無い。**
- **⚠ そして `(j)` が、画像の側と衝突している。****`02` の吹き出しに `3番`**——**日本語で描かれた、型の三種の外にある文字である。****`(j)` の一文（`no other text appears anywhere in the frame`）はいま、これを禁じている。**
    - **⚠ しかしそれは、その一枚でいちばん良い部分である**——**点呼簿の注記が「番号だけ」と既に言っており、吹き出しは同じことを動きで言っている。****情報ではなく、描写である。**
    - **⚠ これは `(i)` と同じ形である**——**私の一文が、抑えたいものの隣を撃つ。** **著者へ戻した（未決）。**
- **`examples/habits/character/README.md` に「課題（未決）」の節を足した**（著者の指示）。**八項目**——画像から出た五つ（`3番` の衝突・多面図の残欠陥・`(j)(k)(l)` 未測・`caption` の診断・画像の無い十七枚）と、構造から出た三つ（〔導出〕の保留・名前の二段・§5b① の担保）。
- SKILL.md の version 0.1.45→0.1.46。
