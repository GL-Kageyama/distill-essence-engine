...
- **⚠️ 画像は一枚も生成していない。切ったのはプロンプトだけである。** 各枚は十の部分（H1／初稿ブロック／メタ四行／出典の語／発明（要承認）／内容／フォーマット／様式＋様式メモ／合成プロンプト／否定行）を持ち、**型は既存の初稿（`_backup_20260916/habits-usui-chinatsu-character-sheet-luminous-anime/prompt.md`）から逐語で採った。**
